...
`blend` is called to generate a result from an array of configured
`BlendingLayer`s. A shader is procedurally generated by using string
manipulation, applied to a `ColorRect` and then rendered using `render_texture`.
"""
