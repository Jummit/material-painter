...
var current_file : ProjectFile setget _set_current_file
var current_layer_material : MaterialLayerStack setget _set_current_layer_material
var undo_redo := UndoRedo.new()
var _generation_context : MaterialGenerationContext
...
# warning-ignore:unsafe_property_access
	...
	file_menu_button.get_popup().connect("id_pressed", self,
...
func _set_current_layer_material(to) -> void:
	current_layer_material = to
	current_layer_material.context = _generation_context
	emit_signal("current_layer_material_changed", to,
			current_file.layer_materials.find(current_layer_material))


func _set_mask(layer, mask : TextureLayerStack) -> void:
	if mask == NO_MASK:
		layer.mask = null
	else:
		layer.mask = mask
...
func _do_change_mask_action(action_name : String, layer,
		mask : TextureLayerStack) -> void:
	undo_redo.create_action(action_name)
	undo_redo.add_do_method(self, "_set_mask", layer, mask)
	undo_redo.add_do_method(layer_tree, "reload")
	undo_redo.add_undo_method(self, "_set_mask", layer, layer.mask)
	undo_redo.add_undo_method(layer_tree, "reload")
	undo_redo.commit_action()


func _do_add_layer(layer : MaterialLayer) -> void:
	var onto
...
func _start_empty_project() -> void:
	_set_current_file(ProjectFile.new({
...
func _set_current_file(to : ProjectFile) -> void:
	...
	var result = _load_mesh(current_file.model_path)
	while result is GDScriptFunctionState:
		result = yield(result, "completed")
	_set_current_layer_material(current_file.layer_materials.front())
	emit_signal("context_changed", _generation_context)
	emit_signal("current_file_changed", current_file)


func _load_project_file(path : String) -> void:
	var file := File.new()
	if file.open(path, File.READ) != OK:
		return
	var result = JSON.parse(file.get_as_text())
	if result.error:
		load_file_error_dialog.dialog_text =\
				"Can't parse json: Content is corrupted.\n At line %s:\n%s" % [
				result.error_line, result.error_string]
		load_file_error_dialog.popup_centered()
		return
	var project := ProjectFile.new(result.result)
	file.close()
	project.path = path
	_set_current_file(project)


const POLLS_PER_FRAME := 30000
func _load_mesh(path : String) -> void:
...
# warning-ignore:unsafe_method_access
	...
	while not new_mesh:
		for _i in POLLS_PER_FRAME:
			new_mesh = interactive_loader.poll()
			if new_mesh:
				break
		var stage := interactive_loader.get_stage()
		progress_dialog.set_action("Stage %s / %s" % [stage, stage_count], stage)
		yield(get_tree(), "idle_frame")
	progress_dialog.complete_task()
	_generation_context.mesh = new_mesh
	current_file.model_path = path
	for surface in _generation_context.mesh.get_surface_count():
	...
	emit_signal("mesh_changed", _generation_context.mesh)
	emit_signal("layer_materials_changed", current_file.layer_materials)
	_set_current_layer_material(current_file.layer_materials.front())


# Save the current project, prompt the user for a file path if the file is saved
# for the first time.
...
# warning-ignore:unsafe_method_access
	...
	for surface in current_file.layer_materials.size():
		var material_name := _generation_context.mesh.surface_get_material(
				surface).resource_name
		var export_folder := current_file.path.get_base_dir().\
...
func _on_SurfaceList_surface_selected(surface : int) -> void:
	_set_current_layer_material(current_file.layer_materials[surface])
...
func _on_AddButton_pressed() -> void:
	_do_add_layer(MaterialLayer.new())
...
func _on_MaterialLayerPopupMenu_mask_removed() -> void:
	_do_change_mask_action("Remove Mask", layer_tree.get_selected_layer(),
			NO_MASK)
...
# warning-ignore:unsafe_method_access
	var progress_dialog = ProgressDialogManager.create_task("Bake Mesh Maps",
			...
			_generation_context.mesh.get_surface_count())
	
	for surface in _generation_context.mesh.get_surface_count():
		var mat_name := _generation_context.mesh.surface_get_material(surface)\
				.resource_name
		for map in mesh_maps_generator.BAKE_FUNCTIONS:
			...
			var result : ImageTexture = yield(
					mesh_maps_generator.generate_mesh_map(map,
					_generation_context.mesh, Vector2(1024, 1024), surface),
					"completed")
...
func _on_EditMenuButton_size_selected(size) -> void:
	_generation_context.result_size = size
	current_layer_material.update()
...
func _on_QuitConfirmationDialog_confirmed() -> void:
	var result = _request_save_file()
	while result is GDScriptFunctionState:
		result = yield(result, "completed")
	_do_quit()


# Callbacks of procedural connected signals
